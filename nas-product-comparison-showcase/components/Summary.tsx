      ...
      <div className="container max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {recommendations.map((item) => (
            <div
              key={item.id}
              className="group relative overflow-hidden rounded-2xl bg-slate-50 border border-slate-200 hover:border-brand-primary/30 hover:shadow-xl transition-all duration-300 p-8"
            >
              <div className="flex items-start gap-6">
                <div
                  className={`p-4 rounded-xl shadow-lg shrink-0 text-white ${item.colorClass} group-hover:scale-110 transition-transform duration-300`}
                >
                  <item.icon size={28} />
                </div>
                <div>
              ...
              <div
                className={`absolute -bottom-10 -right-10 w-32 h-32 rounded-full opacity-10 blur-2xl ${item.colorClass}`}
              >
              </div>
            </div>
          ))}
